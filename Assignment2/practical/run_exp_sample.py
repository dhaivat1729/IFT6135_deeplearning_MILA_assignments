...
if args.model == 'RNN':
    ...
    model.load_state_dict(torch.load('/network/home/bhattdha/IFT6135H20_assignment/Assignment2/practical/Exp1/RNN_SGD_model=RNN_optimizer=SGD_initial_lr=1.0_batch_size=128_seq_len=35_hidden_size=512_num_layers=2_dp_keep_prob=0.8_num_epochs=20_save_best_save_dir=Exp1_0/best_params.pt'))
elif args.model == 'GRU':
    model = GRU(emb_size=args.emb_size, hidden_size=args.hidden_size,
                seq_len=args.seq_len, batch_size=args.batch_size,
                vocab_size=vocab_size, num_layers=args.num_layers,
                dp_keep_prob=args.dp_keep_prob)
    model.load_state_dict(torch.load('/network/home/bhattdha/IFT6135H20_assignment/Assignment2/practical/Exp6/GRU_ADAM_model=GRU_optimizer=ADAM_initial_lr=0.001_batch_size=128_seq_len=35_hidden_size=512_num_layers=2_dp_keep_prob=0.5_num_epochs=20_save_best_save_dir=Exp6_0/best_params.pt'))
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        if args.model == 'TRANSFORMER':
            ...
            x = model.generate(inputs[-1], hidden, 30)
            # import ipdb; ipdb.set_trace()
            outputs, hidden, hidden_list = model(inputs, hidden)
            

        outputs = outputs[outputs.shape[0]-1,:,:].unsqueeze(0)

        # outputs = outputs[outputs.shape[0]-1,:,:]
        targets = torch.from_numpy(y.astype(np.int64)).transpose(0, 1).contiguous().to(device)#.cuda()
        targets = targets[targets.shape[0]-1,:].unsqueeze(0)
        ...
        time_steps, hidden_stuff = list(zip(*hidden_list))
        ## the gradient can now be accessed by hidden_list[0][1].grad
        grad = []
        for i, h in enumerate(hidden_stuff[::2]):
            grad.append(torch.norm(hidden_stuff[2*i].grad + hidden_stuff[2*i+1].grad).item() )


        if grad_overall is None:
            grad_overall = np.array(grad)
        else:
            grad_overall += np.array(grad)

    grad_overall = grad_overall/step
...
def generate_samples(model, data):
    ...
    with torch.no_grad():
        for i in range(10):
            ...
            model.zero_grad()
            hidden = repackage_hidden(hidden)
            sample = model.generate(inputs[0], hidden, model.seq_len).cpu().numpy()
            samples.append(sample)
            
    samples = np.transpose(np.array(samples).reshape(10,35))
...
#     grad_overall_train = run_epoch(model, train_data, True, lr)
    

#     # import ipdb; ipdb.set_trace()
#     # RUN MODEL ON VALIDATION DATA
#     grad_overall_val = run_epoch(model, valid_data)
    

#     np.save('GRU_hidden_grads.npy', {'train_hidden_grads':grad_overall_train, 'val_hidden_grads':grad_overall_val})
#     import ipdb; ipdb.set_trace()
